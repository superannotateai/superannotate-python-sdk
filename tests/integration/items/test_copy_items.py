...
class TestCopyItems(BaseTestCase):
    ...
    def test_copy_items_from_root_with_annotations(self):
        ...
        assert len(skipped_items) == 0
        assert len(sa.list_items(f"{self.PROJECT_NAME}/{self.FOLDER_1}")) == 2
    ...
    def test_copy_items_from_folder(self):
        ...
        assert len(skipped_items) == 0
        assert len(sa.list_items(f"{self.PROJECT_NAME}/{self.FOLDER_2}")) == 7
    ...
    def test_copy_items_wrong_items_list(self):
        ...
        items = sa.list_items(f"{self.PROJECT_NAME}/{self.FOLDER_1}")
        assert len(items) == 1
