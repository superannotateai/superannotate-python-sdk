...
def sa_vector_to_coco_instance_segmentation(
    ...
    for instance in instances:
        ...
        grouped_polygons.setdefault(group_id, {}).setdefault(category_id, []).append(
            points
        )
    ...
    for polygon_group in grouped_polygons.values():
        for cat_id, polygons in polygon_group.items():
            anno_id = next(id_generator)
            masks = _polytoMask(polygons, image_info["height"], image_info["width"])
            mask = _merge(masks)
            area = int(_area(mask))
...
def sa_vector_to_coco_keypoint_detection(
    jsons, id_generator, id_generator_anno, id_generator_img, make_image_info, total_num
):
    ...
    This can happen if the template was deleted from superannotate.com. Ignoring this annotation'
            ...
            if (
                ...
                or instance["templateId"] in template_names
            ):
                continue
        ...
        image_info = make_image_info(
            json_["metadata"]["name"],
            json_["metadata"]["height"],
            json_["metadata"]["width"],
            image_id,
        )
