...
def coco_panoptic_segmentation_to_sa_pixel(coco_path, images_path):
    ...
    for annotate in tqdm(annotate_list, "Converting"):
        ...
        if img_cv is None:
            print("'{}' file dosen't exist!".format(os.path.join(images_path, annot_name + ".png")))
            continue
        ...
        for i, seg in enumerate(segments):
            img[np.all(img == id2rgb(seg["id"]),axis=1)] = _hex_to_rgb(hex_colors[i+1])
            dd = {
        ...
        cv2.imwrite(os.path.join(images_path, annot_name + ".jpg___save.png"), img)
