...
def coco_instance_segmentation_to_sa_vector(coco_path, images_path):
    ...
    for img in tqdm(coco_json['images'], "Writing annotations to disk"):
        ...
        with open(
            os.path.join(
                images_path, image_path + "___objects.json"
            ), "w"
        ) as new_json:
...
def coco_keypoint_detection_to_sa_vector(coco_path, images_path):
    ...
    cat_id_to_cat = {}
    for cat in coco_json["categories"]:
        cat_id_to_cat[cat["id"]] = {"name": cat["name"], "keypoints": cat["keypoints"], "skeleton": cat["skeleton"]}
            ...
            if annot["category_id"] in cat_id_to_cat.keys():
                ...
                for connection in cat_id_to_cat[annot["category_id"]]['skeleton']:
                    ...
                    sa_template['connections'].append(
                        {
                            'id': index + 1,
                            'from': cat_id_to_cat[annot["category_id"]]['skeleton'][index][0],
                            'to': cat_id_to_cat[annot["category_id"]]['skeleton'][index][1]
                        }                        
                    )
